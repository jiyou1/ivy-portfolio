/* Soft holo blobs behind the content (spec §4.1). No runtime filter: blur():
   these are pure radial-gradients that fade to transparent, so they read soft
   without a blur filter. Decorative and inert.

   `blobs` is parameterized so a sibling case study (LatteLearn) can pass its own
   warm café palette while reusing the exact same non-blurred technique; ICOI
...
export default function CaseBlobs({ blobs = ICOI_BLOBS }) {
  return (
    <div aria-hidden="true" className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
      {blobs.map((b, i) => (
        <div key={i} className="absolute rounded-full" style={b} />
      ))}
    </div>
  );
}
